...
The `arkenfox x PrivateVictor user.js` is a **template** which aims to provide as much privacy and enhanced security as possible, and to reduce tracking and fingerprinting as much as possible - while minimizing any loss of functionality and breakage (but it will happen).
...
## ⚪  privatevictor fork
The privatevictor fork added a number of extentions to the browser, note that the user.js file does the job of many other extentions so they are not needed.
It also has tweaked the user.js on
